...
int Objectness::loadTrainedModel(string modelName) // Return -1, 0, or 1 if partial, none, or all loaded
{
	...
	if (!matRead(s1, filters1f) || !matRead(sI, idx1i)){
		//mycout << "... [ Can't load model: " << s1 << " or " << sI << endl;
	...
	if (!matRead(s2, _svmReW1f) || _svmReW1f.size() != Size(2, _svmSzIdxs.size())){
		_svmReW1f = Mat();
		//mycout << "... [ Can't load model: " << s2 << endl;
		printf("Can't load model: %s .\n", _S(s2));
		return -1;
	}
	printf("Success to load models: %s and %s.\n", _S(s1), _S(sI));
	return 1;
}
...
void Objectness::predictBBoxSI(CMat &img3u, ValStructVec<float, Vec4i> &valBoxes, vecI &sz, int NUM_WIN_PSZ, bool fast)
{
    ...
    //std::mycout << _voc.localDir + format("%d.png", r) << std::endl;
...
void Objectness::trainObjectness(int numDetPerSize)
{
  ...
  printf("Start to learn.\n");

  if (CmFile::IsFileExist(_modelName + ".wS1") == false || CmFile::IsFileExist(_modelName + ".wS2") == false)
  {
	  generateTrianData();
  }
  ...
  //mycout << "... [ Learning stage II takes " << tm2.TimeInSeconds() << " seconds... " << endl;
  return;
}
...
#pragma omp parallel for
		...
		tm.Stop();
		printf("Average time for predicting an image (%s) is %g.s\n", _clrName[_Clr], tm.TimeInSeconds() / TestNum);
